...
String nom_wifi; // qui sera lu depuis la EEprom
...
volatile bool top_seconde;  // top_seconde=bit pour la lecture des valeurs toute les secondes
...
int annee;   // année sur deux octets
byte  h_ete; // 1 = heure d'hiver, 2 = heure d'été
...
void setup() {
  ...
  EEPROM.begin(320 + 131); // 320 octets d'utilisé pour stocker les datas 128 pour ssid et mot de pass
  ...
  nom_wifi = EEPROM.readString(321); // nom du réseau SSID
  String mdp  = EEPROM.readString(321 + 64);// mot de passe
    
  WiFi.begin(nom_wifi, mdp); // se connecte au wifi
  ...
  delay(1000);
  lecture_heure_web();      // au demarrage on lit l'heure sans correction été/hiver
  sychro_heure_web();       // lit l'heure depuis internet avec correction 
  ...
  Serial.println(F("fifi82 (montaulab) - 2025"));
  Serial.println( date() );
  Serial.print(F("nom du reseau :")); Serial.println(nom_wifi);
  Serial.print(F("l'adresse ip de l'esp32 est : "));Serial.println( WiFi.localIP() );
  Serial.println(F("Pour configurer l'esp32 :"));
  Serial.println(F("|ssid nom_wifi mot_de_passe| change le nom du wifi et son mot de passe"));
  Serial.println(F("|nom wifi| affiche le nom du reseau wifi"));
  Serial.println(F("|reboot| redemarre l'esp32 "));
}// fin setup()
...
void loop() {
  ...
  if (top_seconde){ // toute les secondes
    top_seconde = false; // bit antirebond pour la lecture toutes les secondes

    if (annee<2025) sychro_heure_web(); // si pas de synchro on recharge l'heure avec le web

    tension_m =   PZEM_maison.voltage();  // lecture de la tension électrique pour info
    if (String(tension_m) == "nan") tension_m = 0;

    w_maison  = PZEM_maison.power();  // lecture des watts de la maison 
    if (String(w_maison) == "nan") w_maison = 0;
    
    tension_s = PZEM_solaire.voltage();  // lecture des watts du solaire
    w_solaire = PZEM_solaire.power();  // lecture des watts du solaire 
    if (String(w_solaire) == "nan") w_solaire = 0;    
    
    wh_maison  += w_maison  / 3600.0; // calcule les watts en une heure (3600s) de la maison 
...
String page_0(){ // page HTML
  ...
  titre( "affichage des données", "h2" , "#33aa55", "#FFFFFF");
  ...
  titre ("tension : " + String(tension_m) + " V" ,"h2","#ffffff", "#0000ff");
 
  bouton( "Récap sur un mois" ,"recap","aaaaaa","24"); br();
...
/*################################################## page récap ###################################################*/
 String page_recap(){ // page html
  ...
  titre( "récap sur un mois", "h2" , "#33aa55", "#FFFFFF");
  ...
  for (int i=1; i<32; i++){
    ...
    if (a<25 or m<1 or m>12) t = " pas de donnée pour cette journée";
    else {
...
/*##################################################  sychro_heure_web ###################################################*/
void sychro_heure_web(){ // calcul heure d'été ou hiver
...
/*##################################################  serie ###################################################*/
void serie(){ // 
  String lecture_serie ="";
  bool in = false;
  while(Serial.available()) {
    lecture_serie+= char(Serial.read());
    in = true;
    }
    
  if (in) {
    int p0 = lecture_serie.indexOf(" ");
    if(p0>0){// si un espace existe dans le texte entré
      String code = lecture_serie.substring(0,p0);
      if(code=="ssid"){
        nom_wifi;
        String mdp; 
        int p1 = lecture_serie.indexOf(" ",p0+1);
        if(p1<0) return;
        nom_wifi = lecture_serie.substring(p0+1,p1);
        int p2 = lecture_serie.indexOf(" ",p1+1);
        if(p1<0) return;
        mdp = lecture_serie.substring(p1+1,p2);        
        Serial.println(F("Changement du reseau wifi :"));
        Serial.print(F("nom du reseau :"));Serial.println(nom_wifi);
        Serial.print(F("mot de passe :"));Serial.println(mdp);
        EEPROM.writeString(321, nom_wifi);
        EEPROM.writeString(321 + 64, mdp);
        EEPROM.commit();
        ESP.restart();
        Serial.print(F("l'adresse ip de l'esp est : "));Serial.println( WiFi.localIP() );
      }// fin if(code=="ssid")
      else if(lecture_serie=="nom wifi"){
        Serial.print(F("nom du wifi : "));Serial.print(nom_wifi);Serial.println();
      }
      else if(lecture_serie=="reboot"){
        ESP.restart();
      }
    }// fin if(p0>0)   
  }// fin else  
