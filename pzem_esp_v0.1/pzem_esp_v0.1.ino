...
(3.2.0 ne fonctionne pas) la version 3.1.3 fonctionne
...
// pzem.resetEnergy();          // reset des kw/h du pzem
...
// mem_cwh en centiemme de kw pour garder un chiffre après la virgule lors de l'affichage
unsigned int mem_cwh[32][6];  // mémorisation des valeurs, date,wh_maison, wh_solaire, wh_solaire_EDF,wh_EDF_maison
...
void setup() {
  ...
  for ( int i=1; i<32; i++) { // 31 jours a d'enregistréer
    ...
    for (int j=0; j<5; j++) { // 5 données à enregistrer de 2 octets
      mem_cwh[i][j] = EEPROM.readUShort( ad + j * 2 ); delay(10);  // "*2" 2 octets par données
    }
  }
...
void loop() {
  ...
  if (pzem){ // toute les secondes
    ...
    float val = w_maison - w_solaire; // regarde si on prend ou on donne à EDF
    if (val < 0) {                    // solaire => EDF
      w_solaire_EDF = -val;           // calcul des watts donné à EDF
      wh_solaire_EDF += w_solaire_EDF/ 3600.0; // calcul les w/h donné à EDF
      w_EDF_maison = 0;
    }
    else {                         // EDF => maison
      w_EDF_maison = val;          // calcul des watts consomé sur EDF     
      wh_EDF_maison += w_EDF_maison/ 3600.0; // calcul des w/h consomé sur EDF 
      w_solaire_EDF = 0;
    }
    ...
    if (minuit and reset_pzem){ // à minuit pile sauvegarde et reset des kwh
      reset_pzem = false;
      int a = annee - 2000;
      mem_cwh[jour][0] = mois + a * 256;
      mem_cwh[jour][1] = wh_maison / 100;
      mem_cwh[jour][2] = wh_solaire / 100;
      mem_cwh[jour][3] = wh_solaire_EDF / 100;
      mem_cwh[jour][4] = wh_EDF_maison / 100;

      int ad =  10 * (jour - 1); // adresse dans la EEprom, 10 octets utilisé par jour
      for (int j=0; j<5; j++) { // 5 données à enregistrer de 2 octets
        EEPROM.writeUShort( ad + j * 2 , mem_cwh[jour][j] );
        delay(10);
      }
...
String page_0(){ // page HTML
  ...
  //bouton(" mise a l'heure ","h","aaaaaa","24"); br();  // affiche le bouton de mise à l'heure // a faire automatiquement
...
/*################################################## page récap ###################################################*/
 String page_recap(){ // page html
  ...
  for (int i=1; i<32; i++){
    int a = mem_cwh[i][0]/256;    // année
    int m = mem_cwh[i][0] - a*256;// mois
    if (a<25 or m<1 or m>12) t = " pas de donnee pour cette journee";
    else {
      t = String(i) + " " +        String( t_mois[m] ) + " " + String( 2000 + a ); // date
      t +=  ", Maison: " +         String( mem_cwh[i][1]/10.0 ) + "kwh "; //  /1000 pour kilowatt, 
      t += "| solaire: " +         String( mem_cwh[i][2]/10.0 ) + "kwh " ;
      t += "| solaire => EDF: " +  String( mem_cwh[i][3]/10.0 ) + "kwh " ; // 
      t += "| EDF  => maison: " +  String( mem_cwh[i][4]/10.0 ) + "kwh " ; // 
    }
